...
class CustomBottomNavBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> navItems = [
      ...
      {'icon': Icons.map_rounded, 'label': 'All Lead'}, 
      {'icon': Icons.store_rounded, 'label': 'Products'}, 
    ...
    return Stack(
      ...
      children: [
        ...
        // Center FAB Button moved down to the bar
        Positioned(
          top: 15, // Adjusted to sit within the bar
          child: GestureDetector(
            onTap: () {
              Get.toNamed(AppRoutes.addLead);
            },
...
// Custom painter for the curved navigation bar (unchanged)
class BottomNavBarPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint()
    ...
    path.moveTo(0, 0);
    path.lineTo(size.width * 0.35, 0);
    path.quadraticBezierTo(size.width * 0.40, 0, size.width * 0.42, 40);
    path.quadraticBezierTo(size.width * 0.0, 70, size.width * 0.0, 40);
...
// Scaffold implementation example (unchanged)
class BottomNavScaffold extends StatelessWidget {
  final Widget body;
